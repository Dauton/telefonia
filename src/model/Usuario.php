...
class Usuario
{
    ...
    {
        $sql = "SELECT *, DATE_FORMAT(data_cadastro, '%d/%m/%Y') AS data_cadastro FROM tb_users";
    ...
    public function cadastraUsuario(string $nome_usuario, string $usuario, string $senha, string $perfil): void
    {
        ...
        $sql = "INSERT INTO tb_users (nome_usuario, usuario, senha, perfil, foto_perfil, status, cadastrado_por, senha_primeiro_acesso) VALUES (?,?,?,?,?, 'Ativado', ?, 'Pendente')";
        $stmt = $this->pdo->prepare($sql);
    ...
    {
        $sql = "SELECT * FROM tb_users WHERE id_usuario = ?";
        $stmt = $this->pdo->prepare($sql);
    ...
    public function editaUsuario(int $id_usuario, string $nome_usuario, string $usuario, string $perfil, string $status): void
    {
        ...
        $sql = "UPDATE tb_users SET nome_usuario = ?, usuario = ?, perfil = ?, status = ? WHERE id_usuario = ?;
    ...
    {
        $sql = "UPDATE tb_users SET senha = ? WHERE id_usuario = ?";
        $stmt = $this->pdo->prepare($sql);
        $senha = password_hash($senha, PASSWORD_ARGON2ID);
    ...
    public function excluiUsuario(int $id_usuario): void
    {
        ...
        if ($resultado <= 0) {
            ...
            // EXCLUI O USUÁRIO...
            $sqlDeletaUsuario = "DELETE FROM tb_users WHERE id_usuario = ?";
