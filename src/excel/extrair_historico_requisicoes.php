...
<head>
    ...
    <body>
        <?php
        $nome_arquivo = 'HISTORICO_DE_REQUISIÇÕES.xls';
        ...
        $html .= '<td style="background: #0088ff"><b>BAIXA</b></td>';
        $html .= '<td style="background: #0088ff"><b>DATA BAIXA</b></td>';
        $html .= '</tr>';

        try {
            $result_tabela = $pdo->query("SELECT * FROM tb_requisicoes_historico ORDER BY id_historico DESC");
            ...
            while ($linha_tabela = $result_tabela->fetch(PDO::FETCH_ASSOC)) {
                ...
                $html .= '<td>' . $linha_tabela["baixa_historico"] . '</td>';
                $html .= '<td>' . $linha_tabela["data_baixa_historico"] . '</td>';
                $html .= '</tr>';
            }
