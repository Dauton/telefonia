...
<head>
    ...
    <body>
        <?php
        $nome_arquivo = 'REGISTROS_LOGINS.xls';

        $html = '';
        $html .= '<table border="1">';
        $html .= '<tr style="color: #ffffff; text-align: center; font-family: Calibri">';
        $html .= '<td style="background: #0088ff"><b>ID registro</b></td>';
        $html .= '<td style="background: #0088ff"><b>Usuário informado</b></td>';
        $html .= '<td style="background: #0088ff"><b>Resultado do evento</b></td>';
        $html .= '<td style="background: #0088ff"><b>Data do evento</b></td>';
        $html .= '</tr>';

        try {
            $result_tabela = $pdo->query("SELECT * FROM tb_logs_login ORDER BY id DESC");

            while ($linha_tabela = $result_tabela->fetch(PDO::FETCH_ASSOC)) {
                $html .= '<tr>';
                $html .= '<td>' . $linha_tabela["id"] . '</td>';
                $html .= '<td>' . $linha_tabela["usuario_informado"] . '</td>';
